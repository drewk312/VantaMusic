/* MD5 for Qobuz/GDStudio signatures — adapted from Joseph Myers' implementation. */
export function md5Hex(input: string): string {
  const bytes = md5(String(input));
  return Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join("");
}

function md5(string: string): number[] {
  let a: number, b: number, c: number, d: number;
  const x = messageToWords(string);
...
function messageToWords(str: string): number[] {
