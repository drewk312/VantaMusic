...
volatile char benchmarkCompletionFlag = 0;
...
void JamesDSPReallocateBlock(JamesDSPLib *jdsp, size_t n)
{
	...
	float *newBuffer = (float *)malloc(ctMemBlk * sizeof(float));
	if (!newBuffer)
		return;
	jdsp->tmpBuffer[0] = newBuffer;
...
void JamesDSPFree(JamesDSPLib *jdsp)
{
	...
	if (jdsp->tmpBuffer[0]) {
		free(jdsp->tmpBuffer[0]);
		jdsp->tmpBuffer[0] = NULL;
		jdsp->tmpBuffer[1] = NULL;
		jdsp->tmpBuffer[2] = NULL;
		jdsp->tmpBuffer[3] = NULL;
		jdsp->tmpBuffer[4] = NULL;
		jdsp->tmpBuffer[5] = NULL;
	}
	int i;
	if (jdsp->blobsCh1[0])
	{
